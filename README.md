...
## Defects:
- Not support Ubuntu 22.04(jammy) and further versions because Canonical deprecated netboot file and force users to use its fuckin Cloud-init, it runs in QEMU environment and CPU hardware virtualization must be demanded but most of VPS doen't work it well at all. Canonical is deserting its' low value devoted fans and discarding a large amount of former precious Debian installer preseed heritances by playing a role as a guilty dictator like Apple.Inc and violating the spirit of the free software.
